...
for k in range(0,5):
    ...
    weight_path ='./COVID19/dct/repeated/fold'+str(k+1)+'/mlp_weight/'  # dct or dwt or pixel
    intermediate_datafolder = './COVID19/dct_intermediate_fold' +str(k+1)+'/' # dct_intermediate_fold or dwt_intermediate_fold or pixel_intermediate_fold
    ...
    x_train, y_train = get_data_label(intermediate_datafolder,train_files)
    x_val, y_val = get_data_label(intermediate_datafolder,val_files)
    x_test, y_test = get_data_label(intermediate_datafolder,test_files)
        
        
    x_train = np.concatenate((np.squeeze(x_train),np.squeeze(x_train),np.squeeze(x_train)),axis = 1)
    x_val = np.concatenate((np.squeeze(x_val),np.squeeze(x_val),np.squeeze(x_val)),axis = 1)
    x_test = np.concatenate((np.squeeze(x_test),np.squeeze(x_test),np.squeeze(x_test)),axis = 1)
    ...
    counter = counter + 12
wb.save('./COVID19/dct/repeated/fold'+str(k+1)+'/result.xls') # dct or dwt or pixel
