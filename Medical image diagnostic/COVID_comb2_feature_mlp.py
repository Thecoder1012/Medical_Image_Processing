...
for k in range(0,5):
    ...
    weight_path ='./COVID19/pixel_dct/fold'+str(k+1)+'/mlp_weight/'  # pixel_dct or pixel_dwt or dct_dwt
    intermediate_datafolder1 = './COVID19/pixel_intermediate_fold' +str(k+1)+'/' # dct_intermediate_fold or dwt_intermediate_fold or pixel_intermediate_fold
    intermediate_datafolder2 = './COVID19/dct_intermediate_fold' +str(k+1)+'/' # dct_intermediate_fold or dwt_intermediate_fold or pixel_intermediate_fold
    ...
    x_test1, y_test = get_data_label(intermediate_datafolder1,test_files)
    x_train1, y_train = get_data_label(intermediate_datafolder1,train_files)
    x_val1, y_val = get_data_label(intermediate_datafolder1,val_files)
    
    x_test2, y_test = get_data_label(intermediate_datafolder2,test_files)
    x_train2, y_train = get_data_label(intermediate_datafolder2,train_files)
    x_val2, y_val = get_data_label(intermediate_datafolder2,val_files)
    ...
    x_train = np.concatenate((np.squeeze(x_train1),np.squeeze(x_train2)),axis = 1)
    x_val = np.concatenate((np.squeeze(x_val1),np.squeeze(x_val2)),axis = 1)
    x_test = np.concatenate((np.squeeze(x_test1),np.squeeze(x_test2)),axis = 1)
    
    trainS = np.concatenate((x_train,x_val),axis = 0)
    labelTr = np.concatenate((y_train,y_val),axis = 0)
    testS = x_test
    labelTs = np.squeeze(y_test)
    del x_train1,x_train2,x_val1,x_val2,x_test1,x_test2,x_train,x_val,x_test
    for it in range(0,10):
        print('fold: ',k,'   iteration: ',it)
    #%% define model
        seed = np.random.randint(0,1000)
        reset_random_seeds(seed)
        model = denseMlpCreate_comb2(seed)
    ...
    counter = counter + 12
    wb.save('./COVID19/pixel_dct/fold'+str(k+1)+'/result.xls') # pixel_dct or pixel_dwt or dct_dwt
