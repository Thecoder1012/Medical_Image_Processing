...
class RespiratoryDisease private constructor(
        ...
        private val respiratoryDiseaseStateHandler: RespiratoryDiseaseStateHandler
) {
    ...
    interface RespiratoryDiseaseStateHandler {
        fun restoreRespiratoryDiseaseState(): Bundle
        fun saveRespiratoryDiseaseState(state: Bundle?)

        companion object DefaultStateHandler : RespiratoryDiseaseStateHandler {
            override fun restoreRespiratoryDiseaseState() = Bundle()
            override fun saveRespiratoryDiseaseState(state: Bundle?) {}
        }
    }
    ...
    private fun startCameraForImage(caller: Any) {
        ...
        getCallerActivity(caller)?.let { activityCaller ->
            ...
            val capableComponent = activityCaller.startActivityForResult(takePictureIntent, RequestCodes.TAKE_PICTURE);
...
//                    }
            ...
            if (capableComponent == null) {
                Log.e(RESPIRATORYDISEASE_LOG_TAG, "No app capable of handling camera intent")
                cleanup()
            }
        }
    }
    ...
    fun handleActivityResult(requestCode: Int, resultCode: Int, resultIntent: Intent?, activity: Activity, callbacks: Callbacks) {
        // RespiratoryDisease request codes are set to be between 374961 and 374965.
    ...
    private fun onPickedExistingPicturesFromLocalStorage(resultIntent: Intent, activity: Activity, callbacks: Callbacks) {
        Log.d(RESPIRATORYDISEASE_LOG_TAG, "Existing picture returned from local storage")
        try {
    ...
    private fun onPickedExistingPictures(resultIntent: Intent, activity: Activity, callbacks: Callbacks) {
        try {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
                ...
                if (clipData != null) {
                    Log.d(RESPIRATORYDISEASE_LOG_TAG, "Existing picture returned")
                    ...
                    if (files.isNotEmpty()) {
                        ...
                        callbacks.onImagePickerError(RespiratoryDiseaseException("No files were returned from gallery"), MediaSource.GALLERY)
                    }
                    cleanup()
                } else {
    ...
    private fun onPictureReturnedFromCamera(activity: Activity, callbacks: Callbacks) {
        Log.d(RESPIRATORYDISEASE_LOG_TAG, "Picture returned from camera")
        lastCameraFile?.let { cameraFile ->
            try {
                ...
                error.printStackTrace()
                callbacks.onImagePickerError(RespiratoryDiseaseException("Unable to get the picture returned from camera.", error), MediaSource.CAMERA_IMAGE)
            }
        }
        cleanup()
    }
    ...
    private fun onVideoReturnedFromCamera(activity: Activity, callbacks: Callbacks) {
        Log.d(RESPIRATORYDISEASE_LOG_TAG, "Video returned from camera")
        lastCameraFile?.let { cameraFile ->
            try {
                if (cameraFile.uri.toString().isEmpty()) Intents.revokeWritePermission(activity, cameraFile.uri)
                val files = mutableListOf(cameraFile)
//                if (copyImagesToPublicGalleryFolder) Files.copyFilesInSeparateThread(activity, folderName, files.map { it.file })
    ...
    private fun onFileReturnedFromChooser(resultIntent: Intent?, activity: Activity, callbacks: Callbacks) {
        Log.d(RESPIRATORYDISEASE_LOG_TAG, "File returned from chooser")
    ...
    private fun removeCameraFileAndCleanup() {
        lastCameraFile?.file?.let { file ->
            Log.d(RESPIRATORYDISEASE_LOG_TAG, "Removing camera file of size: ${file.length()}")
            file.delete()
            Log.d(RESPIRATORYDISEASE_LOG_TAG, "Clearing reference to camera file")
            lastCameraFile = null
            save()
        }
    }
    ...
    private fun cleanup() {
        lastCameraFile?.let { cameraFile ->
            Log.d(RESPIRATORYDISEASE_LOG_TAG, "Clearing reference to camera file of size: ${cameraFile.file.length()}")
            lastCameraFile = null
            save()
        }
    }
    ...
    private fun save() {
        RespiratoryDiseaseStateHandler.saveRespiratoryDiseaseState(
                Bundle().apply {
    ...
    private fun restore() {
        RespiratoryDiseaseStateHandler.restoreRespiratoryDiseaseState().apply {
    ...
    class Builder(private val context: Context) {
        companion object {
            private fun getAppName(context: Context): String = try {
                ...
                Log.e(RESPIRATORYDISEASE_LOG_TAG, "App name couldn't be found. Probably no label was specified in the AndroidManifest.xml. Using RespiratoryDisease as a folder name for files.")
                error.printStackTrace()
                "RespiratoryDisease"
            }
        }
        ...
        private var respiratoryDiseaseStateHandler: RespiratoryDiseaseStateHandler = RespiratoryDiseaseStateHandler.DefaultStateHandler
        ...
        fun setStateHandler(respiratoryDiseaseStateHandler: RespiratoryDiseaseStateHandler): Builder {
            this.respiratoryDiseaseStateHandler = respiratoryDiseaseStateHandler
            return this
        }

        fun build(): RespiratoryDisease {
            return RespiratoryDisease(
                    ...
                    respiratoryDiseaseStateHandler = respiratoryDiseaseStateHandler
            )
        }
    }
}